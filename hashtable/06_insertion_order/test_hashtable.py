...
def test_should_iterate_over_keys(hash_table):
    assert list(hash_table.keys) == ["hola", 98.6, False]


def test_should_iterate_over_values(hash_table):
    assert list(hash_table.values) == ["hello", 37, True]


def test_should_iterate_over_pairs(hash_table):
    assert list(hash_table.pairs) == list(
        zip(hash_table.keys, hash_table.values)
    )
