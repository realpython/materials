...
def test_should_wrap_index_around_when_collides(mock_hash):
    hash_table = HashTable(capacity=3, load_factor_threshold=1)
    hash_table["hola"] = "hello"
    hash_table[98.6] = 37
    hash_table[False] = True
...
def test_should_include_deleted_in_load_factor(hash_table):
    del hash_table["hola"]
    del hash_table[98.6]
    assert hash_table.load_factor == 3 / 100
...
def test_should_reclaim_deleted_slots_after_rehash():
    hash_table = HashTable(capacity=100, load_factor_threshold=0.5)
    for i in range(50):
        hash_table[i] = "value"
    ...
    assert len(hash_table) == 26
    assert hash_table.capacity == 200
    assert hash_table.load_factor == 26 / 200
