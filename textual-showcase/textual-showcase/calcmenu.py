...
class MenuItem(HorizontalGroup):
    def __init__(self, text:str, id:str, unit_price:float) -> None:
...
class CalcMenuApp(App):
    ...
    ITEMS = [
        ...
        [":beer: Beer", "beer", 4.50],
        [":wine_glass: Wine", "wine", 6.25],
        [":tropical_drink: Cocktail", "cocktail", 8.50],
    ]   
    def compose(self) ->ComposeResult:
        yield Header()
    ...
    def update_grand_total (self):
        grand_total = 0
        for item in self.ITEMS:
            grand_total+= self.query_one(f"#{item[1]}").total
